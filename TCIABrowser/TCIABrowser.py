...
class TCIABrowserWidget:
  ...
  def setup(self):
    ...
    patientsTreeSelectionModel = self.patientsTreeView.selectionModel()
    ...
    studiesTreeHeaderLabels = ['Study Date','Study ID','Study Description','Admitting Diagnosis Descrition','Patient Age','Collection','Series Count','Study Instance UID']
    self.studiesModel.setHorizontalHeaderLabels(studiesTreeHeaderLabels)
  ...
  def onRequestButton(self):
    ...
    currentIndex = self.collectionsTreeSelectionModel.currentIndex().row() 
    selectedCollection = self.collectionsItems[currentIndex].text()
    try:    
  ...
  def populatePatientsTreeView(self,responseString):
    ...
    for patient in patients:
      patientID = qt.QStandardItem(str(patients[n]['PatientID']))
      patientName = qt.QStandardItem(str(patients[n]['PatientName']))
      # patientBirthDate = qt.QStandardItem(str(patients[n]['PatientBirthDate']))
      patientSex = qt.QStandardItem(str(patients[n]['PatientSex']))
      # ethnicGroup = qt.QStandardItem(str(patients[n]['EthnicGroup']))
      collection = qt.QStandardItem(str(patients[n]['Collection']))
      self.patientsIDs.append(patientID)
      self.patientNames.append(patientName)
      # self.patientBirthDates.append(patientBirthDates)
      self.patientSexes.append(patientSex)
      # self.ethnicGroups.append(ethnicGroups)
      self.collections.append(collection)
      model.setItem(n,0,patientID)
      #root.appendRow(patientID)
      model.setItem(n,1,patientName)
      model.setItem(n,2,patientSex)
      model.setItem(n,3,collection)
      n +=1
    '''
